...
extern "C" {
  ...
  void initc_output_cal_(int *nwpc_,int *ndep_,double*pvkd_,double*pebdep_){
    md.nwpc=*nwpc_;
    //md.pvkd=HNEWN(double,(md.nwpc+1)*IMDPS*kl);
    //CPYN(md.pvkd,pvkd_,(md.nwpc+1)*IMDPS*kl);
    md.pvkd=pvkd_;
    md.ndep =*ndep_;
  ...
  void mean1_(double *ea,int *iacb,int *iace, float *ape ,float *tpf ,float *aet ,float *h1_3){
    ...
    for(iac=*iacb;iac<*iace;iac++){
      mean1_1(ea+iac*mkj,md.pvkd+iac*IMDPS*kld,ape+iac,tpf+iac,aet+iac,h1_3+iac);
    }
  }
  void mean1t_(double *ea,int *iac_,float*apet,float*tpft,float*aett,float*h1_3t){
    int iac=*iac_;
    mean1_1(ea+iac*mkj,md.pvkd+iac*IMDPS*kld,apet,tpft,aett,h1_3t);
  }
  ...
  void mixture_(double *ea,int *iacb,int *iace,float*bv){
    int iac;
    for(iac=*iacb;iac<*iace;iac++){
      mixture_1(ea+iac*mkj,md.pebdep+iac*md.npdep*kld,bv+iac,md.nwpc+1);
    }
  }
