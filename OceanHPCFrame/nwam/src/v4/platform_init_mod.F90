  ...
  do ko=1,Navhists
    if(avhists(ko)%hist_ect/=0)then
      ...
      if(ind>=0)then
        ...
        IF(ph%wrwa>0)THEN
          call mean1(ti,ea,iacb,iace,ph%ape,ph%tpf,ph%aet,ph%h1_3) 
          call mean1(ti,ea,iacb2,iace2,ph%ape,ph%tpf,ph%aet,ph%h1_3) 
        endif
        IF(ph%wrbv>0)THEN
          call mixture(ti,ea,iacb,iace,ph%bv) ! bv
          call mixture(ti,ea,iacb2,iace2,ph%bv) ! bv
        endif
        if(ph%mean/=0)then
        ...
        if(ind==0)then
          call gwaitsubs(ti,RFB);
          call gsetmain(ti,RFB);
        else
          call ssetgrp(ti,RFB);
        endif
      else
