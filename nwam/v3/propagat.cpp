...
void InitCPropgats(){
  ...
  long size=mkj/8L*(gppar.nwpc+1);
  pvis=HNEWN(struct v_interg ,size);
  int ires=hbw_verify_memory_region(pvis, size*sizeof (struct v_interg),  1);
  if(ires){
    printf(" Hbm alloc pvis error %ld %p============ \n",size,pvis);
...
#if 0
void propagats_naive(double *wav_e,double *wav_ee,int iacb,int iace){
  int iac;
